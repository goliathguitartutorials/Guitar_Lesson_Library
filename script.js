 ...
 * @param {string} buttonGroupClass - The class name shared by buttons in the same group (e.g., 'sort-button', 'decade-button').
 */
function activateFilterButton(button, buttonGroupClass) {
    const activeButtons = document.querySelectorAll(`.${buttonGroupClass}.active`);
...
function renderLessons(lessonsArray) {
    ...
    lessonDiv.classList.add('lesson');
     if (index % 2 !== 0) {
        lessonDiv.classList.add('lesson-reverse');
    }
    lessonDiv.innerHTML = `
        <div class="lesson-content">
            <div class="lesson-info">
                <h2>${lesson.title}</h2>
                <p><span class="genre">Genre: ${lesson.genre}</span></p>
                <p>${lesson.description}</p>
            </div>
            <a href="${lesson.patreonLink}" target="_blank">Access Full Lesson</a>
        </div>
        <div class="video-container" data-video-id="${lesson.videoId}">
            <img src="https://img.youtube.com/vi/${lesson.videoId}/hqdefault.jpg" alt="Video Thumbnail">
...
if (selectedSort) {
  switch (selectedSort) {
    case 'recent':
        filteredLessons.sort((a, b) => new Date(b.publishedDate) - new Date(a.publishedDate));
        break;
    case 'popular':
         filteredLessons.sort((a, b) => b.viewCount - a.viewCount);
        break;
    case 'a-z':
...
if (selectedDecade && selectedDecade !== "All") {
    filteredLessons = filteredLessons.filter(lesson => lesson.decade === selectedDecade)
}
 ...
 filterButtons.forEach(button => {
    button.addEventListener('click', function() {
        if (this.classList.contains('sort-button')) {
            activateFilterButton(this, 'sort-button');
            selectedSort = this.getAttribute('data-sort');
         } else if (this.classList.contains('decade-button')) {
            activateFilterButton(this, 'decade-button');
             selectedDecade = this.getAttribute('data-filter');
              } else if (this.classList.contains('difficulty-button')) {
            activateFilterButton(this, 'difficulty-button');
             selectedDifficulty = this.getAttribute('data-filter');
        }
